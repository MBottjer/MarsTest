...
describe Rover do 
  ...
  let(:position) {double(:x_coord => 0, :y_coord => 0, :facing => "N")}
  let(:rover) {Rover.new(position)}

  context 'the rover knows its position' do 
  ...
  context 'it is able to take commands' do 
